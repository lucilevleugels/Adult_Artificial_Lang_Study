...
def training_block(win, training_phase_text, train_df, choice_data, iteration):
    ...
    if control[0] == 'space':
        ...
        for i,row in enumerate(train_df.iterrows(),1):
            ...
            # Presenting Images & Playing Sound
            tk.sendMessage(f'Sound {trial_data[condition_column]} On_set')
            tk.sendMessage(f'!V TRIAL_VAR Audio-Train {trial_data[condition_column]}')
            sound_stim.play()
...
def testing_block(win, testing_phase_test, test_df, choice_data, iteration ):
    ...
    if control[0] == 'space':
        ...
        for i,row in enumerate(test_df.iterrows(),1):
            ...
            tk.sendMessage(f'Sound {trial_data[condition_column]} On_set')
            tk.sendMessage(f'!V TRIAL_VAR Audio-Test {trial_data[condition_column]}')
            sound_stim.play()
            tk.sendMessage(f'Sound {trial_data[condition_column]} End')
